...
namespace RentAll.ConsoleApp
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            Console.WriteLine($"Gross leasable area of {centerRepository.FindCenterById(1).Name} is: {centerService.CalculateGrossLeasableAreaPerCenter(1)} sqm");

            Console.WriteLine($"Occupancy degree of {centerRepository.FindCenterById(1).Name} is: {Math.Round(centerService.CalculateOcupancyDegreePerCenter(1), 2)}%");
            Console.WriteLine($"Average rent of {centerRepository.FindCenterById(1).Name} is: {Math.Round(centerService.CalculateAverageRentPerSQMPerCenter(1), 2)} EUR/SQM");
